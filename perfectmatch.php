...
<body>
    ...
    <div class="title">
        ...
        <p>Discover all the beautiful connections waiting for you.</p>
        <p class="romantic-line">"In a world full of stories, yours is the one I want to be a part of forever."</p>
    </div>
    ...
    <div class="container">
        ...
        if ($result_user_details->num_rows > 0) {
         ...
         // Get the logged-in user's religion
$sql_user_religion = "SELECT religion FROM userdetails WHERE username = ?";
$stmt_user_religion = $conn->prepare($sql_user_religion);
$stmt_user_religion->bind_param("s", $username);
$stmt_user_religion->execute();
$result_user_religion = $stmt_user_religion->get_result();

if ($result_user_religion->num_rows > 0) {
    $user_religion = $result_user_religion->fetch_assoc()['religion'];
} else {
    echo "Error fetching religion data.";
    exit;
}
        ...
        // Set age range (2 years older or younger)
        $min_age = $user_age - 2;
        $max_age = $user_age + 2;

     // Prepare SQL query to find matching users based on opposite gender, age range, and religion
$sql = "SELECT first_name, last_name, age, city, state, occupation, gender, username, religion, verified 
FROM userdetails 
WHERE gender = ? 
AND age BETWEEN ? AND ? 
AND (city = ? OR state = ? OR occupation = ?)
AND religion = ? 
ORDER BY RAND() 
LIMIT 10";  // Fetch 10 random users

// Prepare and execute the SQL statement
$stmt = $conn->prepare($sql);
$stmt->bind_param("siiisss", $opposite_gender, $min_age, $max_age, $user_city, $user_state, $user_occupation, $user_religion);
$stmt->execute();
...
<!-- JavaScript for displaying the verification message -->
<script>
